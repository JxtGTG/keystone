...
class Trainer:
    ...
    def train(
        ...
        **kwargs,
        ...
        # Backup weights for 'restore' strategy before training starts
        self._save_restore_strategy_weights()
        ...
        try:
            if args.push_to_hub:
                try:
                    ...
                    train_result = inner_training_loop(
                        args=args,
            ...
            # In restore mode, do not restore weights at the end of training
            # Because each saved checkpoint already contains the restored version
            if (hasattr(self.args, "neuron_update_strategy") and 
                self.args.neuron_update_strategy == "restore"):
                logger.info("🔄 Restore mode: training finished; each saved checkpoint contains the restored version")
                # Cleanup memory after training
                if hasattr(self, 'weights_to_restore'):
                    del self.weights_to_restore
                if hasattr(self, 'initial_weights_json'):
                    del self.initial_weights_json
    ...
    def _inner_training_loop(
        ...
        # Save trained weights before restoration (for restore strategy)
        # self._save_trained_weights_before_restore()
    ...
    def _save_checkpoint(self, model, trial):
        ...
        if (hasattr(self.args, "neuron_update_strategy") and 
            self.args.neuron_update_strategy == "restore" and 
            hasattr(self.args, "activate_neuron") and 
            self.args.activate_neuron is not None and
            hasattr(self, "weights_to_restore") and 
            self.weights_to_restore):
            
           
            restore_checkpoint_folder = f"{PREFIX_CHECKPOINT_DIR}-{self.state.global_step}-restore"
            restore_output_dir = os.path.join(run_dir, restore_checkpoint_folder)
            
            
            os.makedirs(restore_output_dir, exist_ok=True)
            
            
            self._save_training_weights_for_restore()
            
            
            self._restore_restore_strategy_weights()
            
           
            self.save_model(restore_output_dir, _internal_call=True)
        ...
        # If in restore mode, copy other files to the restore version
        if (hasattr(self.args, "neuron_update_strategy") and 
            self.args.neuron_update_strategy == "restore" and 
            hasattr(self.args, "activate_neuron") and 
            self.args.activate_neuron is not None and
            hasattr(self, "weights_to_restore") and 
            self.weights_to_restore and
            not self.args.save_only_model):
            
            import shutil
            # Copy optimizer and scheduler files
            for file_name in ["optimizer.bin", "scheduler.bin", "scaler.pt", "rng_state.pth"]:
                src_file = os.path.join(output_dir, file_name)
                dst_file = os.path.join(restore_output_dir, file_name)
                if os.path.exists(src_file):
                    shutil.copy2(src_file, dst_file)
            
            # Copy trainer state
            src_state_file = os.path.join(output_dir, TRAINER_STATE_NAME)
            dst_state_file = os.path.join(restore_output_dir, TRAINER_STATE_NAME)
            if os.path.exists(src_state_file):
                shutil.copy2(src_state_file, dst_state_file)
    ...
    def _get_kv_div_factor(self):
        ...
        try:
            ...
            nkh = int(getattr(config, "num_key_value_heads", nh))
            ...
            return int(factor) if abs(factor - int(factor)) < 1e-6 else 1
            
        except Exception as e:
          
            logger.warning(f"Failed to calculate KV div factor: {e}, using default value 1")
            return 1
    ...
    #     kv_factor = self._get_kv_div_factor()
    ...
    #                 # Handle FSDP flattened parameters (1D gradients)
    #                 if param.grad.dim() == 1:
    #                     # For 1D gradients, create mask for entire flattened gradient
    #                     mask = torch.ones(param.grad.shape[0], dtype=torch.bool, device=param.device)
    #                     indices = [i // div_factor for i in tune_idx]
    #                     # Filter out indices that are out of bounds for this shard
    #                     valid_indices = [idx for idx in indices if idx < param.grad.shape[0]]
    #                     if valid_indices:
    #                         mask[valid_indices] = False
    #                     param.grad[mask] = 0
    #                 else:
    #                     # For 2D gradients, handle normally
    #                     index_dim = 1 if proj_key == "down_proj" or proj_key == "attn.o_proj" else 0
    #                     # Use param.grad.shape instead of param.size() for FSDP compatibility
    #                     mask = torch.ones(param.grad.shape[index_dim], dtype=torch.bool, device=param.device)
    #                     indices = [i // div_factor for i in tune_idx]
    #                     # Filter out indices that are out of bounds for this shard
    #                     valid_indices = [idx for idx in indices if idx < param.grad.shape[index_dim]]
    #                     if valid_indices:
    #                         mask[valid_indices] = False
    ...
    def mask_gradients(self, model, activate_neuron):
        ...
        n_kv_heads = getattr(cfg, "num_key_value_heads", None) if cfg is not None else None
        ...
        def _map_q_to_kv_indices(idx_set):
          
            if not isinstance(idx_set, (set, list, tuple)) or head_dim is None or n_heads is None or n_kv_heads is None:
                
                return list(idx_set)

            kv_factor = max(1, n_heads // max(1, n_kv_heads))
            mapped = set()
            limit = n_kv_heads * head_dim
        ...
        for name, param in model.named_parameters():
            ...
            for proj_key, (neuron_key, div_factor) in proj_map.items():
                if proj_key in name and "bias" not in name:
                    ...
                    if neuron_key in ("attn_k", "attn_v"):
                        tune_idx = set(_map_q_to_kv_indices(tune_idx))
                    ...
                    if param.grad.dim() == 1:
                        ...
                        if neuron_key in ("attn_k", "attn_v"):
                            ...
                            indices = [i for i in tune_idx if 0 <= i < param.grad.shape[0]]
                        else:
                            indices = [i // div_factor for i in tune_idx]
                            indices = [i for i in indices if 0 <= i < param.grad.shape[0]]
                        ...
                        index_dim = 1 if proj_key in ("down_proj", "attn.o_proj") else 0
                        dim_len = param.grad.shape[index_dim]
                        mask = torch.ones(dim_len, dtype=torch.bool, device=param.device)

                        if neuron_key in ("attn_k", "attn_v"):
                            indices = [i for i in tune_idx if 0 <= i < dim_len]
                        else:
                            indices = [i // div_factor for i in tune_idx]
                            indices = [i for i in indices if 0 <= i < dim_len]
    ...
    def training_step(
        ...
        if self.use_apex:
            ...
            # Use accelerator.backward directly; let accelerator handle DeepSpeed
            self.accelerator.backward(loss, **kwargs)
            
            # Only apply gradient masking when using 'freeze' strategy
            if (
                hasattr(self.args, "neuron_update_strategy")
                and self.args.neuron_update_strategy == "freeze"
                and hasattr(self.args, "activate_neuron")
                and self.args.activate_neuron is not None
            ):
                ...
                # For DeepSpeed, we need to ensure gradients are properly synchronized
                if hasattr(self.accelerator, 'distributed_type') and self.accelerator.distributed_type.name == 'DEEPSPEED':
                    # Force gradient synchronization in DeepSpeed
    ...
    def _match_neuron_key_to_param_name(self, neuron_key, param_name):
        """
        neuron_key: e.g. 'fwd_up', 'fwd_down', 'attn_q', 'attn_k', 'attn_v', 'attn_o'
        param_name: e.g. 'model.layers.0.mlp.up_proj.weight'
        """
        mapping = {
            'fwd_up': 'up_proj',
            'fwd_down': 'down_proj', 
        ...
        if neuron_key in mapping:
            return mapping[neuron_key] in param_name
        return False

    def _get_target_param_names(self, neuron_key, all_param_names):
        """
        """
        mapping = {
            'fwd_up': 'up_proj',
        ...
        if neuron_key not in mapping:
            return []
        
        target_pattern = mapping[neuron_key]
        # Use list comprehension to filter all matching parameters at once
        return [name for name in all_param_names if target_pattern in name]

    def _extract_layer_index(self, param_name):
        """
        Extract layer index from parameter name.
        Example: 'model.layers.5.mlp.up_proj.weight' -> 5
        """
        import re
    ...
    def _save_restore_strategy_weights(self):
        """
        Saves the weights of the neurons specified in `activate_neuron` if the strategy is 'restore'.
        This should be called before the training loop starts.
        """
        # Check if the restore strategy is active and neurons are specified
        if (
            hasattr(self.args, "neuron_update_strategy")
            and self.args.neuron_update_strategy == "restore"
            and hasattr(self.args, "activate_neuron")
            and self.args.activate_neuron is not None
        ):
            import json
            ...
            kv_div_factor = self._get_kv_div_factor()

            self.weights_to_restore = {}
            self.initial_weights_json = {}  
            model = self.model
            activate_neuron = self.args.activate_neuron

            logger.info("Backing up weights for 'restore' strategy...")
            logger.info(f"Activate neuron keys: {list(activate_neuron.keys())}")

          
            param_dict = {name: param for name, param in model.named_parameters()}
            logger.info(f"Total model parameters: {len(param_dict)}")

           
            target_param_names = set()
            for neuron_key in activate_neuron.keys():
                target_param_names.update(self._get_target_param_names(neuron_key, param_dict.keys()))
            logger.info(f"Target parameters to backup: {len(target_param_names)}")

           
            for neuron_key, layer_neurons in activate_neuron.items():
             
                matching_param_names = self._get_target_param_names(neuron_key, param_dict.keys())

                for param_name in matching_param_names:
                    ...
                    if "mlp.down_proj.weight" in param_name  or "self_attn.o_proj.weight" in param_name:
                        index_dim = 1  # column
                        div_factor = 1
                    else:
                        index_dim = 0  # row
                        if ("self_attn.k_proj.weight" in param_name and "bias" not in param_name) or ("self_attn.v_proj.weight" in param_name and "bias" not in param_name):
                            div_factor = kv_div_factor
                        else:
                            div_factor = 1

                 
                    layer_idx = self._extract_layer_index(param_name)
                    if str(layer_idx) not in layer_neurons:
                        continue

                    neuron_indices = layer_neurons[str(layer_idx)]

                    
                    if param_name not in self.weights_to_restore:
                        self.weights_to_restore[param_name] = {}
                        self.initial_weights_json[param_name] = {}
 
                    for neuron_idx in neuron_indices:
                        orig_key = int(neuron_idx)          
                        idx = orig_key // div_factor         

                        try:
                            if index_dim == 0:
                             
                                if 0 <= idx < param.shape[0]:
                                    vec = param.data[idx].detach().cpu().clone()
                                    self.weights_to_restore[param_name][orig_key] = vec
                                    self.initial_weights_json[param_name][orig_key] = vec.float().numpy().tolist()
                                else:
                                    logger.warning(f"Skip backup: {param_name} row index {idx} out of range")
                            else:
                               
                                if 0 <= idx < param.shape[1]:
                                    vec = param.data[:, idx].detach().cpu().clone()
                                    self.weights_to_restore[param_name][orig_key] = vec
                                    self.initial_weights_json[param_name][orig_key] = vec.float().numpy().tolist()
                                else:
                                    logger.warning(f"Skip backup: {param_name} col index {idx} out of range")
                        except Exception as e:
                            logger.warning(f"Backup failed: {param_name}[{orig_key}] -> {e}")

         
        

    def _restore_restore_strategy_weights(self):
        """
        Restores the saved weights to the specified neurons if the strategy is 'restore'.
        This should be called after the training loop ends and before the model is saved.
        """
        # Check if the restore stra
                    #tegy was used and if weights were saved
        if (
            hasattr(self.args, "neuron_update_strategy")
            and self.args.neuron_update_strategy == "restore"
            and hasattr(self, "weights_to_restore")
            and self.weights_to_restore
        ):
            logger.info("Restoring weights for 'restore' strategy...")

            kv_div_factor = self._get_kv_div_factor()

          
            param_dict = {name: param for name, param in self.model.named_parameters()}
            for param_name, neuron_weights in self.weights_to_restore.items():
                if param_name not in param_dict:
                    continue
                ...
                # Only restore weights for specified neurons
                for neuron_idx, saved_weight in neuron_weights.items():
                    idx = int(neuron_idx) // div_factor  # k/v scaling; others not scaled
                    try:
                        if index_dim == 0:
                            # Row restore: param[idx, :] ← saved_weight
                            # Use param.shape for FSDP compatibility
                            if 0 <= idx < param.shape[0]:
                                vec = saved_weight.to(param.device)
                                if vec.shape != param.data[idx].shape:
                                    vec = vec.view_as(param.data[idx])
                                param.data[idx].copy_(vec)
                                logger.info(f"  - Restored neuron {idx} (row) in {param_name}")
                            else:
                                logger.warning(f"  - Skip restore: {param_name} row index {idx} out of range")
                        else:
                            # Column restore: param[:, idx] ← saved_weight
                            # Use param.shape for FSDP compatibility
                            if 0 <= idx < param.shape[1]:
                                vec = saved_weight.to(param.device)
                                if vec.dim() > 1:
                                    vec = vec.view(-1)
                                if vec.numel() != param.shape[0]:
                                    logger.warning(
                                        f"  - Skip {param_name}[{idx}] due to shape mismatch: "
                                        f"expect {param.shape[0]}, got {vec.numel()}"
                                    )
                                    continue
                                param.data[:, idx].copy_(vec)
                                logger.info(f"  - Restored neuron {idx} (col) in {param_name}")
                            else:
                                logger.warning(f"  - Skip restore: {param_name} col index {idx} out of range")
                    except Exception as e:
                        logger.warning(f"  - Failed to restore {param_name}[{idx}]: {e}")

                logger.info(f"  - Completed restoration for: {param_name}")

            logger.info("Weight restoration complete.")
          


    def _save_training_weights_for_restore(self):
     
        if not hasattr(self, "weights_to_restore") or not self.weights_to_restore:
            return
        ...
        for param_name, neuron_weights in self.weights_to_restore.items():
            ...
            self.training_weights[param_name] = {}
            for neuron_idx in neuron_weights.keys():
                ...
                try:
                    if index_dim == 0:
                        ...
                        if 0 <= idx < param.shape[0]:
                            self.training_weights[param_name][orig_key] = param.data[idx].detach().cpu().clone()
                    else:
                       
                        if 0 <= idx < param.shape[1]:
                            self.training_weights[param_name][orig_key] = param.data[:, idx].detach().cpu().clone()
                except Exception as e:
                    logger.warning(f" {param_name}[{orig_key}] -> {e}")

       

    def _restore_training_weights(self):
       
        if not hasattr(self, "training_weights") or not self.training_weights:
            return

        
        
        # Restore post-training weights
        param_dict = {name: param for name, param in self.model.named_parameters()}
        for param_name, neuron_weights in self.training_weights.items():
            if param_name in param_dict:
                ...
                if ("self_attn.k_proj.weight" in param_name and "bias" not in param_name) or ("self_attn.v_proj.weight" in param_name and "bias" not in param_name):
                    div_factor = self._get_kv_div_factor()
                else:
                    div_factor = 1
        ...
        logger.debug("✅ Training-state weights restored")
